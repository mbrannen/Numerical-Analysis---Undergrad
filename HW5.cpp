...
int main() {
	...
	if(funSelector(fun) == 1){
			...
			if(/* valN%2 == 0 && */ valN != 0){
				...
				for(int i = 1; i <= (valN2); i++){
					iHelp = i;
					if((i%2) == 1){
						oddFunSum = oddFunSum + f1(intA + (iHelp*valH2));
					}
				}
				for(int i = 1; i <= (valN2 - 1 ); i++){
					iHelp = i;
					if((i%2) == 0){
						evenFunSum = evenFunSum + f1((intA + (iHelp*valH2)));
					}
					
				}	
				answer = (valH2/3)*(f1(intA) + f1(intB) + (4 * oddFunSum) + (2 * evenFunSum));
	...
	if(funSelector(fun) == 2){
			...
			while (evenCheck == false) {
				...
				for(int i = 1; i <= (valN2); i++){
					iHelp = i;
					if((i%2) == 1){
						oddFunSum = oddFunSum + f2(intA + (iHelp*valH2));
					}
				}
				for(int i = 1; i <= (valN2 - 1 ); i++){
					iHelp = i;
					if((i%2) == 0){
						evenFunSum = evenFunSum + f2((intA + (iHelp*valH2)));
					}
					
				}	
				answer = (valH2/3)*(f2(intA) + f2(intB) + (4 * oddFunSum) + (2 * evenFunSum));
